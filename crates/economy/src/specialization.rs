...
pub fn production_efficiency(
    advantage: &RegionAdvantage,
    demand_pressure: f64,
) -> f64 {
    let base = advantage.resource_multiplier
        * advantage.infrastructure_level
        * advantage.skill_base;
...
pub fn determine_specialization(
    ...
    let mut scored: Vec<(String, f64)> = advantages.iter()
        .map(|(good_id, adv)| {
    ...
    let production_focus: Vec<String> = scored.iter()
        .take(max_focus)
        .map(|(id, _)| id.clone())
        .collect();
...
pub fn comparative_advantage(
    region_a: &RegionAdvantage,
    region_b: &RegionAdvantage,
) -> f64 {
