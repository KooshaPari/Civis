...
struct ServerMetrics {
    ...
    /// Reason-coded registry removals and socket receiver exits. Kept separate
    /// from the legacy aggregate so a full control lane is distinguishable from
    /// a peer close or transport error in a live incident.
    ws_client_removals: IntCounterVec,
...
impl ServerMetrics {
    fn new(registry: &Registry) -> Result<Self, String> {
        ...
        let ws_client_removals = IntCounterVec::new(
            Opts::new(
                "civ_ws_client_removals",
                "WebSocket client registry removals and socket exits by reason",
            ),
            &["reason"],
        )
        .map_err(|e| format!("create civ_ws_client_removals: {e}"))?;
        ...
        registry
            .register(Box::new(ws_client_removals.clone()))
            .map_err(|e| format!("register civ_ws_client_removals: {e}"))?;
        registry
...
async fn replay_import(
    ...
    *state.sim.lock().await = loaded;
    ...
    clients.retain(|tx| match tx.control.try_send(reset.clone()) {
        Ok(()) => true,
        Err(tokio::sync::mpsc::error::TrySendError::Full(_)) => {
            // This removes the outbound registry entry but does not itself close
            // the TCP socket. Record it separately so live diagnosis does not
            // mistake a saturated control lane for a peer-initiated close.
            state.metrics.ws_client_disconnects.inc();
            state
                .metrics
                .ws_client_removals
                .with_label_values(&["replay_reset_control_full"])
                .inc();
            tracing::warn!("pruning websocket client after replay import: control lane full");
            false
        }
        Err(tokio::sync::mpsc::error::TrySendError::Closed(_)) => {
            ...
            state
                .metrics
                .ws_client_removals
                .with_label_values(&["replay_reset_control_closed"])
                .inc();
            tracing::info!("pruning websocket client after replay import: control lane closed");
            false
        }
    });
    Ok((
...
async fn handle_socket(
            ...
            tokio::select! {
                ...
                control = control_rx.recv() => {
                    let Some(control) = control else {
                        tracing::info!(%connection_id_for_forward, "ws forward task ended: control lane closed");
                        return;
                    };
                    if let Err(error) = sender.send(control).await {
                        tracing::warn!(%connection_id_for_forward, %error, "ws forward task ended: control send failed");
                        return;
                    }
                }
                changed = tick_rx.changed() => {
                    if let Err(error) = changed {
                        tracing::info!(%connection_id_for_forward, %error, "ws forward task ended: tick lane closed");
                        return;
                    }
                    ...
                    if !forward_latest_tick(
                        ...
                        tracing::warn!(%connection_id_for_forward, "ws forward task ended: tick send failed");
                        return;
                    }
                }
            }
    ...
    let close_reason = loop {
        let Some(msg) = receiver.next().await else {
            tracing::info!(%connection_id, "ws receiver ended: peer EOF");
            break "peer_eof";
        };
        match msg {
            Ok(Message::Text(text)) => {
                ...
                .await;
                match tx.control.try_send(Message::Text(response)) {
                    Ok(()) => {}
                    Err(tokio::sync::mpsc::error::TrySendError::Full(_)) => {
                        tracing::warn!(%connection_id, "ws receiver ended: response control lane full");
                        break "response_control_full";
                    }
                    Err(tokio::sync::mpsc::error::TrySendError::Closed(_)) => {
                        tracing::info!(%connection_id, "ws receiver ended: response control lane closed");
                        break "response_control_closed";
                    }
                }
            }
            Ok(Message::Close(frame)) => {
                tracing::info!(%connection_id, ?frame, "ws receiver ended: peer close frame");
                break "peer_close";
            }
            Ok(_) => {}
            Err(error) => {
                tracing::warn!(%connection_id, %error, "ws receiver ended: transport error");
                break "receiver_error";
            }
        }
    };
    forward.abort();
