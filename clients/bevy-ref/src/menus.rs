...
fn decode_texture(ctx: &egui::Context, name: &str, bytes: &[u8]) -> Option<egui::TextureHandle> {
    ...
    if data.len() < w * h * 4 {
        error!("menu texture '{name}' pixel buffer too small");
        return None;
    }
    let color = egui::ColorImage::from_rgba_unmultiplied([w, h], &data[..w * h * 4]);
    Some(ctx.load_texture(name, color, egui::TextureOptions::LINEAR))
    ...
    egui::Area::new(egui::Id::new("menu_backdrop"))
        ...
        .show(ctx, |ui| {
            ui.painter().rect_filled(
                screen,
                egui::CornerRadius::ZERO,
                egui::Color32::from_rgba_premultiplied(8, 10, 18, 245),
            );
        });
}
...
fn image_backdrop(ctx: &egui::Context, tex: Option<&egui::TextureHandle>, id: &str) {
    ...
    egui::Area::new(egui::Id::new(id))
        ...
        .show(ctx, |ui| {
            let painter = ui.painter();
            painter.rect_filled(screen, egui::CornerRadius::ZERO, egui::Color32::from_rgb(8, 10, 18));
            let img = tex.size_vec2();
            let scale = (screen.width() / img.x).max(screen.height() / img.y);
            let drawn = img * scale;
            let rect = egui::Rect::from_center_size(screen.center(), drawn);
...
fn main_menu_panel(
    ...
    liquid_glass_frame(ui, RADIUS_PANEL, |ui| {
        ui.set_min_width(340.0);
        ui.vertical_centered(|ui| {
            menu_logo(ui, logo, 220.0);
            ui.label(egui::RichText::new("A civilisation sandbox").size(13.0).color(DIM));
            ui.add_space(28.0);
            main_menu_buttons(ui, command, saves, settings_open, progress, params);
        });
    });
}
...
fn main_menu_buttons(
    ...
    if primary_button(ui, "🌍  New World").clicked() {
        params.randomize();
        progress.reset();
        command.action = MainMenuCommand::NewWorld;
    }
    ui.add_space(8.0);
    let continue_label = if saves.can_continue { "🏃  Continue" } else { "🏃  Continue (no save)" };
    if ui
        .add_enabled(
            saves.can_continue,
...
fn world_setup_panel(ui: &mut egui::Ui, params: &mut WorldSetupParams) {
    egui::Frame::NONE
        .fill(PANEL_FILL)
        ...
        .show(ui, |ui| {
            ...
            ui.add_space(12.0);
            ui.label(egui::RichText::new(format!("Seed: {}", params.seed)).monospace());
            ui.add_space(8.0);
            if menu_button(ui, "🎲  Randomize").clicked() {
                params.randomize();
            }
        });
}

fn loading_panel(ui: &mut egui::Ui, progress: &LoadingProgress, spinner: Option<&egui::TextureHandle>) {
    egui::Frame::NONE
        .fill(PANEL_FILL)
        .corner_radius(egui::CornerRadius::same(14))
        .stroke(egui::Stroke::new(1.5, ACCENT.gamma_multiply(0.45)))
        .inner_margin(egui::Margin::same(28))
        .show(ui, |ui| {
            ui.set_min_width(420.0);
            ui.vertical_centered(|ui| {
                loading_spinner(ui, spinner, progress.fraction);
                ui.add_space(10.0);
                ui.label(egui::RichText::new("Loading World").size(26.0).color(ACCENT).strong());
                ui.label(egui::RichText::new(progress.label.as_str()).color(DIM));
                ui.add_space(12.0);
                gradient_progress_bar(ui, progress.fraction.clamp(0.0, 1.0), 320.0);
                ui.add_space(8.0);
                ui.label(egui::RichText::new(tip_for_frame(progress.fraction * 30.0)).color(egui::Color32::from_rgb(182, 190, 210)).italics());
            });
        });
}

fn loading_spinner(ui: &mut egui::Ui, spinner: Option<&egui::TextureHandle>, elapsed: f32) {
    if let Some(tex) = spinner {
        let size = tex.size_vec2();
        let angle = elapsed * std::f32::consts::TAU;
        let (rect, _) = ui.allocate_exact_size(size.min(egui::vec2(88.0, 88.0)), egui::Sense::hover());
        ui.painter().image(tex.id(), rect, egui::Rect::from_min_max(egui::pos2(0.0,0.0), egui::pos2(1.0,1.0)), egui::Color32::WHITE);
        ui.painter().circle_stroke(rect.center(), rect.width()/2.0, egui::Stroke::new(1.0, ACCENT.gamma_multiply(0.4)));
        ui.painter().line_segment([rect.center(), rect.center() + egui::vec2(angle.cos(), angle.sin()) * (rect.width()/2.0)], egui::Stroke::new(2.0, GOLD));
    }
}
...
fn gradient_progress_bar(ui: &mut egui::Ui, frac: f32, width: f32) {
    ...
    ui.painter().rect_filled(rect, egui::CornerRadius::same(6), CHIP_FILL);
    let filled = egui::Rect::from_min_max(rect.min, egui::pos2(rect.min.x + rect.width() * frac, rect.max.y));
    ui.painter().rect_filled(filled, egui::CornerRadius::same(6), ACCENT);
}
...
fn themed_button(
    ...
    let bg = if hovered { if primary { accent.gamma_multiply(0.34) } else { CHIP_HOVER } } else { fill };
    painter.rect_filled(rect, radius, bg);
    painter.rect_stroke(rect, radius, egui::Stroke::new(if hovered { 1.6 } else { 1.0 }, if hovered { accent } else { accent.gamma_multiply(0.35) }), egui::StrokeKind::Inside);
    let txt = if hovered || primary { egui::Color32::WHITE } else { egui::Color32::from_rgb(214, 224, 240) };
    painter.text(rect.center(), egui::Align2::CENTER_CENTER, label, egui::FontId::proportional(16.0), txt);
    resp
}
