...
//!
//! Surface exposed to `engine.rs`:
//! - [`EmotionDrivenBehavior`] enum: the four states a psyche can produce.
//! - [`behavior_from_psyche`]: deterministic mapping from a `civ_agents::Psyche`
//!   read to one of the four variants.
//!
//! Engine-locality: this is a pure read of an agent's psyche; no mutation
//! is performed. The engine then maps the resulting
//! `EmotionDrivenBehavior` to a concrete [`crate::engine::AgentAction`]
//! via `action_from_emotion_behavior` (defined inline in `engine.rs`).
//!
//! Implementation note: the signal computation reads only the
//! `Mood { valence, arousal }` and `Temperament::impulsivity` fields;
//! `drives` and `beliefs` are non-load-bearing for this slice and are
//! reserved for the FR-CIV-CULTURE follow-up.
...
///
/// The engine maps each variant to a concrete [`crate::engine::AgentAction`]
/// inside `phase_psyche_behavior`. The mapping is documented inline at
/// `crate::engine::action_from_emotion_behavior`.
