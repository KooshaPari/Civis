...
- **CLI `--format json`** — `status`, `query`, `resources`, `override`, `dump`, `reload`, `screenshot`, `component-map` commands now accept `--format json` for machine-readable output; `CommandOutput` helper provides `WriteJson`/`WriteJsonError`/`CreateFormatOption`/`IsJson` utilities; errors suppress ANSI markup when `--format json` is active
...
- **Comprehensive test coverage audit** — Analyzed test suite structure: 1286 total tests (1265 unit + 21 integration); identified gaps in SDK services (ContentRegistrationService, FileDiscoveryService, UniverseLoader), Bridge Protocol edge cases, PackCompiler services (AssetValidation, AssetImport, Optimization), and Scenario domain validators; created coverage planning document COVERAGE_EXPANSION_TASK.md; documented major test gap areas and priority recommendations for reaching 400+ test target; added scripts/coverage.ps1 for automated coverage reporting with Cobertura output
- **UI Domain Plugin (M4d)** — fourth domain plugin completes domain architecture (Warfare, Economy, Scenario, UI); includes 3 models (HudElementDefinition, MenuDefinition, MenuItemDefinition, ThemeDefinition) with full XML doc comments; 3 registries (HudElementRegistry, MenuRegistry, ThemeRegistry) with filtering/hierarchy validation; UIContentLoader for YAML pack loading from hud_elements/, menus/, themes/ directories; UIPlugin entry point exposing all registries and ContentLoader; comprehensive validation for broken menu parent references and menu cycles; ThemeRegistry with built-in dark and light themes and active theme tracking; 38 unit tests covering model construction, registry CRUD, filtering by type/visibility/hierarchy, menu hierarchy validation with cycle detection, theme activation, and pack validation; all code compiles with 0 errors and 0 warnings
...
- **Scenario Domain Plugin (M4c)** — third domain plugin completes core triarchy (Warfare, Economy, Scenario); includes ScenarioRegistry with filtering by difficulty/objective type; ScenarioContentLoader for loading scenario YAML definitions from pack directories; updated ScenarioPlugin to expose Scenarios registry and ContentLoader; 29 new unit tests (20 for ScenarioRegistry, 9 for ScenarioContentLoader) covering registration, lookup, filtering, content loading from disk, malformed input handling, and nested directory traversal; brings Scenario test total to 99 methods across 4 test files (ScenarioTests, ScenarioRunnerTests, ScenarioRegistryTests, ScenarioContentLoaderTests)

### Fixed

- **F9/F10 key input regression** (RT-003, RT-004) — Fixed event handler capture issue in `RuntimeDriver.Initialize()`; handlers were capturing the driver instance (`this`) instead of looking up the current driver from `Plugin.PersistentRoot`; after resurrection, lambdas referenced stale fields of destroyed driver; changed to static handler registration with dynamic current-driver lookup via `GetComponent<RuntimeDriver>()`; made `_uguiReady`, `_dfCanvas`, `_debugOverlay`, `_modMenuHost` `internal` for handler access
