...
///
/// Prefers `(i64) -> i32` exports that receive the simulation tick (FR-CIV-TACTICS-044);
...
pub fn invoke_policy_tick(wasm_bytes: &[u8], sim_tick: u64) -> Result<i32, WasmGuestError> {
    ...
    if let Ok(func) = instance.get_typed_func::<i64, i32>(&mut store, "civlab_policy_tick") {
        return Ok(func.call(&mut store, tick_arg)?);
    }
    if let Ok(func) = instance.get_typed_func::<i64, i32>(&mut store, "policy_tick") {
        return Ok(func.call(&mut store, tick_arg)?);
    }
