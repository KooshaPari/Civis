//! Deterministic grid pathfinding for the operational layer (FR-CIV-TACTICS-033).
...
fn greedy_step(from: (i32, i32), to: (i32, i32)) -> Option<(i32, i32)> {
    let dx = (to.0 - from.0).clamp(-1, 1);
    let dy = (to.1 - from.1).clamp(-1, 1);
    if dx == 0 && dy == 0 {
        None
    } else {
        Some((from.0 + dx, from.1 + dy))
    }
}
