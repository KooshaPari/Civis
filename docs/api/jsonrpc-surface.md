...
| `sim.emergence` | — | `{}` or omit | Latest emergence sample when available; else `{ "tick", "sample": null }` | Unit: `sim_emergence_*` in `jsonrpc.rs` |`r`n| `sim.perf` | — | `{}` or omit | `{ "tick", "last_tick_ms", "agent_count", "ca_steps", "stub" }` | Unit/dispatch coverage in `jsonrpc.rs` |`r`n| `sim.outcome` | — | `{}` or omit | `{ "outcome", "reason", "tick" }` | Unit/dispatch coverage in `jsonrpc.rs` |
...
| `sim.reset` | — | `{ "seed": <u64> }` **required** | `{ "seed": <u64>, "tick": 0 }` | [`ws_jsonrpc_sim_reset_replaces_simulation_and_zeroes_tick`](../../crates/server/tests/ws_smoke.rs) |`r`n| `sim.load_scenario` | — | `{ "preset": <string>, "seed"?: <u64> }` | `{ "preset", "seed", "tick": 0 }` | Unit/dispatch coverage in `jsonrpc.rs` |
...
| `sim.place_voxel` | **operator** | `{ "x", "y", "z": <i64 world>, "material"? }` default `0` | Dispatch: `{ "accepted": true }`; bridge: `{ "accepted", "ok": true }` | — (unit: `parse_place_voxel_params_reads_coords` in `jsonrpc.rs`) |`r`n| `sim.inspect_tile` | — | `{ "x"?: <i64>, "y"?: <i64> }` | `{ "x", "y", "stub": true }` | Unit/dispatch coverage in `jsonrpc.rs` |`r`n| `sim.god_action` | **operator** | `{ "action", "x"?, "y"?, "target_faction"?, "magnitude"? }` | `{ "ok": true, "tick" }` | Unit/dispatch coverage in `jsonrpc.rs` |`r`n| `sim.diplomacy_action` | — | `{ "action"?, "target_faction"? }` | `{ "action", "target_faction", "stub": true, "tick" }` | Unit/dispatch coverage in `jsonrpc.rs` |
