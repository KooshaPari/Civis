...
mod tests {
    ...
    fn powder_high_angle_no_spread() {
        ...
        g.saturation[g.index(1, 2, 0).unwrap()] = 200;
    ...
    fn liquid_blocked_by_denser_material() {
        let mut g = CaGrid::new([3, 2, 1]);
        // Water cannot swap into stone (lower density).
        g.set_with_temp(1, 1, 0, WATER, 20);
        g.set(0, 1, 0, STONE);
        g.set(2, 1, 0, STONE);
        g.mark_dirty_cell(1, 1, 0);
        step_n_with_config(&mut g, reg(), 3, BoundaryConfig::closed(), 0);
        // Water is trapped, stays liquid.
        assert_eq!(g.get(1, 1, 0), WATER);
    }
    ...
    /// Covers snow phase transition (like ice, melts to water).
    #[test]
    fn snow_melts_above_freezing() {
        let mut g = CaGrid::new([1, 1, 1]);
        g.set_with_temp(0, 0, 0, SNOW, 20);
        g.mark_dirty_cell(0, 0, 0);
        step_n_with_config(&mut g, reg(), 1, BoundaryConfig::closed(), 0);
        let result = g.get(0, 0, 0);
        assert_eq!(result, WATER, "SNOW at 20°C must melt to WATER, got {result:?}");
    }
    ...
    fn lava_boils_to_fire_when_hot() {
        let mut g = CaGrid::new([1, 1, 1]);
        g.set_with_temp(0, 0, 0, LAVA, 2000);
        g.mark_dirty_cell(0, 0, 0);
        step_n_with_config(&mut g, reg(), 1, BoundaryConfig::closed(), 0);
        let result = g.get(0, 0, 0);
        assert_eq!(
            result, crate::material::FIRE,
            "LAVA at high temp must boil to FIRE, got {result:?}"
        );
    }
    ...
    /// Covers steam drift in evaporation_pass when no cold neighbor.
    #[test]
    fn steam_drifts_away_when_warm() {
        ...
        g.mark_dirty_cell(1, 1, 0);
        step_n_with_config(&mut g, reg(), 5, BoundaryConfig::closed(), 0);
        // Steam should drift or be replaced; not stay in place.
        let steam_pos = g.get(1, 1, 0);
        assert!(
            steam_pos != STEAM || count(&g, STEAM) > 1,
            "warm steam must drift away or spawn more steam"
        );
    }
    ...
    fn percolation_oversaturates_and_drains() {
        ...
        g.set(1, 1, 0, SAND);
        g.saturation[g.index(1, 1, 0).unwrap()] = 255;
        g.mark_dirty_cell(1, 1, 0);
        step_n_with_config(&mut g, reg(), 3, BoundaryConfig::closed(), 0);
    ...
    fn integer_cube_root_edge_cases() {
        ...
        assert_eq!(integer_cube_root(u32::MAX), 2048);
    }
    ...
    fn read_neighbor_closed_boundary() {
        ...
        let result = read_neighbor(&g, 0, 0, 0, 0, &boundary); // Direction +X at edge
        assert_eq!(result, None, "closed boundary returns None");
    }
    ...
    fn write_back_world_empty_chunks() {
        use crate::{VoxelWorld, FIXED_SCALE, WorldCoord};
        let mut world: VoxelWorld<MaterialId> = VoxelWorld::new(FIXED_SCALE);
    ...
    fn phase_transition_unknown_material() {
        let mut g = CaGrid::new([1, 1, 1]);
        g.cells[0] = MaterialId::from(9999u16); // Unknown ID
        g.temperatures[0] = 100;
        g.mark_dirty_cell(0, 0, 0);
        step_n_with_config(&mut g, reg(), 1, BoundaryConfig::closed(), 0);
        // Unknown material should be skipped (unchanged).
        assert_eq!(g.cells[0], MaterialId::from(9999u16));
    }
    ...
    fn powder_step_with_saturation_bonus() {
        ...
        g.saturation[g.index(0, 2, 0).unwrap()] = 255;
        g.mark_dirty_cell(0, 2, 0);
        step_n_with_config(&mut g, reg(), 3, BoundaryConfig::closed(), 0);
    ...
    fn evaporation_no_air_targets() {
        let mut g = CaGrid::new([1, 1, 1]);
        // Fully enclosed hot water, no air to evaporate into.
        g.set_with_temp(0, 0, 0, WATER, 150);
        step_n_with_config(&mut g, reg(), 2, BoundaryConfig::closed(), 0);
        // Water may stay (no air target) or stay liquid.
        assert_eq!(count(&g, WATER), 1, "water must be conserved");
    }
    ...
    fn molten_metal_boils_to_fire() {
        let mut g = CaGrid::new([1, 1, 1]);
        g.set_with_temp(0, 0, 0, MOLTEN_METAL, 2000);
        g.mark_dirty_cell(0, 0, 0);
        step_n_with_config(&mut g, reg(), 1, BoundaryConfig::closed(), 0);
        let result = g.get(0, 0, 0);
        assert_eq!(
            result, crate::material::FIRE,
            "MOLTEN_METAL at high temp must boil to FIRE"
        );
    }
