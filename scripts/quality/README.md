...
The manifest records a `git_sha`, a blake2b `content_hash` over tracked non-manifest tree entries, and a blake2b `manifest_hash` over gate statuses. Cloud verify rejects stale or tampered manifests. The content hash lets a required squash merge verify the source tree even when the pre-squash commit is unreachable in the default-branch checkout.
