...
| 6 | `docs/design/DIPLOMACY_EMERGENCE.md` + ADR-015 | Faction emergence via k-means, tension, treaty graph | `crates/diplomacy/` | Crate is in the workspace and `phase_diplomacy` is in tick. `sim.diplomacy_action` now validates/acknowledges supported actions, but accepted actions still do not mutate persistent diplomacy substrate state. |
...
| 2 | **`sim.inspect_tile` now projects snapshot readout, but lacks direct voxel material/height** | `crates/server/src/jsonrpc.rs` (per `MCP_CONTROL_SURFACE.md` §2.1.3) | The RPC no longer returns `{ "stub": true }`; it returns normalized tile coordinates, nearest faction, and nearest civ alignment from the current spectator snapshot. Remaining gap: bridge a live engine tile probe for material and terrain height instead of `null`. |
| 3 | **`sim.diplomacy_action` validates verbs but does not mutate diplomacy substrate yet** | `crates/server/src/jsonrpc.rs` | Propose_treaty / declare_war / offer_trade now return non-stub, validated action records with target-faction knowledge. Remaining gap: route accepted actions into persistent diplomacy state/events instead of acknowledgement-only responses. |
...
| 10 | Can a player **declare war**? | Partially. `sim.diplomacy_action` validates and acknowledges `declare_war`; persistent diplomacy-state mutation remains open. | `crates/server/src/jsonrpc.rs` |
...
| 5 | **Complete `sim.inspect_tile` with live voxel material/height** — the JSON-RPC handler now returns a real spectator-derived readout (`nearest_faction`, `nearest_agent_alignment`) instead of a stub marker. Remaining work: fetch `material` and `terrain_height` from a live engine tile probe instead of returning `null`. | The MCP `sim_inspect_tile` and in-game `inspect.probe` can now identify nearby factions/civs, but terrain/material legibility is still incomplete. | `[wire]` | `crates/server/src/jsonrpc.rs`, `crates/engine/src/` (add `Simulation::inspect_tile` if missing) |
| 6 | **Route accepted `sim.diplomacy_action` records into diplomacy substrate** — the JSON-RPC method now validates `propose_treaty` / `declare_war` / `offer_trade` and returns a non-stub action record. Remaining work is persistent mutation through `civ-diplomacy`/engine state. | Agents and clients now get a validated response contract; the missing part is making diplomacy actions alter the simulation. | `[impl]` | `crates/server/src/jsonrpc.rs`, `crates/diplomacy/src/`, `crates/engine/src/` |
...
3. **PR-C (impl): Finish `sim.inspect_tile` material/height + persist `sim.diplomacy_action` effects** — inspect now returns real spectator-derived faction/civ data; diplomacy now validates and acknowledges supported actions but still needs substrate mutation. Unlocks full MCP `sim_inspect_tile` + `sim_get_diplomacy`.
