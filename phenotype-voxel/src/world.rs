...
impl<T: Default + Clone + PartialEq> VoxelWorld<T> {
    ...
    fn axis_span(&self, axis: fn(ChunkCoord) -> i32) -> i64 {
        let mut min = None::<i32>;
        let mut max = None::<i32>;
        for coord in self.chunks.keys().chain(self.octree.nodes.keys()) {
        ...
        i64::from(max - min + 1) * self.voxel_span * i64::from(CHUNK_EDGE as i32)
    }

    /// Approximate occupied world width in fixed-point units.
    #[must_use]
    pub fn width(&self) -> i64 {
        self.axis_span(|coord| coord.cx)
    }

    /// Approximate occupied world depth in fixed-point units.
    #[must_use]
    pub fn depth(&self) -> i64 {
        self.axis_span(|coord| coord.cz)
    }
