...
impl CivisMcpServer {
    ...
    async fn civis_build(
    ...
    ) -> Result<BuildResult, McpError> {
        let args = build_cli_args("build", target_dir);
        let raw = run_cli_json(&args).await?;
        let exit_code = raw
            .get("exit")
        ...
        Ok(BuildResult { exit_code, log_path })
    }
    ...
    ) -> Result<ScreenshotResult, McpError> {
        let _ = run_cli_json::<Value>(&["screenshot".into(), "--out".into(), out.clone()]).await?;

        let bytes = fs::read(&out)
            .await
            .map_err(|err| McpError::internal_error(err.to_string()))?;

        Ok(ScreenshotResult { path: out, bytes })
    }

    #[tool(name = "civis_census", description = "Read latest parsed census JSON")]
    async fn civis_census(&self, _params: Parameters<serde_json::Value>) -> Result<Value, McpError> {
        run_cli_json::<Value>(&["census".into()]).await
    }
    ...
    async fn civis_verify(
        ...
        let raw = run_cli_json::<Value>(&args).await?;
        ...
        let census = raw.get("census").cloned().unwrap_or_else(Value::Null);
...
async fn run_cli_json<T>(args: &[String]) -> Result<T, McpError>
where
    T: serde::de::DeserializeOwned,
{
    ...
    if stdout.is_empty() {
        return Err(McpError::internal_error("civis-cli produced no stdout"));
    }

    serde_json::from_str(&stdout)
        .map_err(|err| McpError::internal_error(format!("invalid JSON stdout: {err}. stdout={stdout}")))
}
...
async fn main() {
    ...
    let transport = stdio();
    let service = CivisMcpServer.serve(transport).await;
    match service {
        Ok(service) => {
