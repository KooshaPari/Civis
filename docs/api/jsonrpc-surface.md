...
| `sim.spawn_entity` | **operator** | `{ "kind", "x", "y", "faction"? }` — `kind`: `civilian` \| `vehicle` \| `airport` \| `port` \| `hangar` | Dispatch: `{ "accepted": true, "kind": <wire label> }`; bridge adds `ok`, `entity_id`; building kinds also add `authoritative`, `tick`, `scene_generation`, `building_graph_version` for the published `BuildingDiff` | [`ws_jsonrpc_sim_spawn_entity_vehicle_returns_entity_id`](../../crates/server/tests/ws_smoke.rs), [`palette_aliases_spawn_authoritative_buildings_and_publish_world_coordinates`](../../crates/server/tests/building_command.rs) |
| `sim.place_voxel` | **operator** | `{ "x", "y", "z": <i64 world>, "material"? }` default `0` | Dispatch: `{ "accepted": true }`; bridge: `{ "accepted", "ok": true, "writes": 1, "authoritative": true, "tick", "scene_generation", "building_graph_version" }` | [`building_palette_and_interleaved_terrain_survive_replay_without_duplication`](../../crates/server/tests/building_command.rs); unit: `parse_place_voxel_params_reads_coords` in `jsonrpc.rs` |
| `sim.terraform_extent` | **operator** | `{ "x", "y", "z": <i64>, "op"?: string default `raise`, "material"?: u16, "radius"?: 1–32 default `3` }` | Dispatch: `{ "accepted": true, "op" }`; bridge: `{ "accepted", "ok", "writes", "op", "authoritative": true, "tick", "scene_generation", "building_graph_version" }` | [`ws_terraform_writes_material_visible_in_binary_chunk`](../../crates/server/tests/voxel_payload.rs); malformed material/radius are rejected by `jsonrpc.rs` unit tests |
