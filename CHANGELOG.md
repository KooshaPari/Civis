...
### Fixed
- **Star Wars main-menu wallpaper: removed baked-in loading bar (BUG A).** `packs/warfare-starwars/assets/ui/menu_bg.png` was rasterized from a *loading-screen* SVG (`ui/loading-republic.svg`), so a static "LOADING MODULE" caption bar + frozen ~68% progress bar were baked into the bottom of the settled main-menu background — a fake progress bar with no load in progress. Added a dedicated clean source `assets/svg/menu-bg.svg` (boot-splash composition minus the lower-third caption/progress layer), re-rendered `menu_bg.png` from it, and repointed `scripts/art/rasterize-menu-takeover.ps1` so the menu wallpaper can never again be generated from a loading SVG.
- **Star Wars loading sequence: live progress layer instead of fake static skeleton (BUG B).** The loading screen previously showed a static image whose progress bar was baked into the background (`assets/loading/loading-bg.png`, identical to the old menu wallpaper) while `LoadingScreenController` pinned the branded overlay on-screen for a fixed `MinVisibleSeconds = 12s` — together producing a ~20s frozen "skeleton" that then handed off to the native DINO loader. Fixes:
  - **Clean themed loading background** — new clean source `assets/svg/ui/loading-bg-clean.svg` (loading-republic minus the baked LAYER 6 caption/progress bar); `loading-bg.png` re-rendered from it. The baked bar is gone, so the runtime progress layer is the only progress indicator.
  - **Live progress layer (`src/Runtime/UI/LoadingScreenController.cs`)** — the progress bar now animates continuously: when no real pack/scene progress has been reported it runs an *indeterminate* ramp easing toward (never reaching) a 92% ceiling, plus a travelling highlight/shimmer, so it never reads as frozen. Real `SetPackProgress` overrides the ramp; the bar snaps to 100% only when the load is confirmed done.
  - **Synced to cover the whole load** — `MinVisibleSeconds` cut from 12s → 1.5s (just enough to avoid a flash); the fade-out is driven by the real readiness signal (`BeginFadeOut`, called once the MainMenu scene + engine UI are ready), and `EnsureVisible` re-arms the live animation on each new load window so the native DINO loader cannot flash through.
  - **Menu-suppressed** — the overlay still fades out and self-destroys once MainMenu is ready, so it never lingers on the settled main menu.
