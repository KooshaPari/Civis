...
use civ_agents::psyche::{belief_culture_exposure, update_beliefs, update_mood, Mood, Temperament};
...
fn n5_cooperated_raises_familiarity_more_than_coexisted() {
    ...
    for _ in 0..5 {
        ...
        apply_social_event(
            ...
            SocialEvent {
                a: 1,
                b: 2,
                kind: Interaction::Coexisted,
                tick: 0,
            },
        );
    }
    let fam_coop = graph_coop
        .ties
        .iter()
        .find(|t| t.other == 2)
        .map(|t| t.familiarity)
        .unwrap_or(0.0);
    let fam_coex = graph_coex
        .ties
        .iter()
        .find(|t| t.other == 2)
        .map(|t| t.familiarity)
    ...
    assert!(
        fam_coop > fam_coex,
        "Cooperated must build more familiarity than Coexisted: coop={fam_coop}, coex={fam_coex}"
    );
}
...
fn n5_coexisted_builds_nonzero_familiarity() {
    ...
    assert!(
        fam > 0.0,
        "10 Coexisted events must produce positive familiarity, got {fam}"
    );
}
...
fn n6_sated_needs_produce_positive_valence() {
    ...
    for _ in 0..20 {
        update_mood(&mut mood, &needs, &temp, 0.0, 0.0, 0.0);
    }
    assert!(
        mood.valence > 0.0,
        "sated needs must produce positive valence, got {}",
        mood.valence
    );
}
...
fn n6_high_threat_raises_arousal() {
    ...
    assert!(
        mood.arousal > 0.5,
        "threat_pressure=1.0 must produce arousal > 0.5, got {}",
        mood.arousal
    );
}
...
fn n6_deprived_needs_produce_negative_valence() {
    ...
    let temp = Temperament::neutral();
    for _ in 0..20 {
        update_mood(&mut mood, &needs, &temp, 0.0, 0.5, 0.0);
    }
    assert!(
        mood.valence < 0.0,
        "fully deprived needs must produce negative valence, got {}",
        mood.valence
    );
}
...
fn n8_high_sociability_converges_beliefs_faster() {
    ...
    let dist_high: f32 = beliefs_high
        .iter()
        .zip(&exposure)
        .map(|(b, e)| (b - e).abs())
        .sum::<f32>()
        / 4.0;
    let dist_low: f32 = beliefs_low
        .iter()
        .zip(&exposure)
        .map(|(b, e)| (b - e).abs())
        .sum::<f32>()
    ...
    assert!(
        dist_high < dist_low,
        "high sociability must converge faster: dist_high={dist_high}, dist_low={dist_low}"
    );
}
...
fn n8_zero_sociability_no_directed_drift() {
    ...
    for _ in 0..50 {
        update_beliefs(&mut beliefs, [0.9, 0.8, 0.7, 0.6], 0.0, &mut rng);
    }
    let drift: f32 = beliefs
        .iter()
        .zip(&initial)
        .map(|(b, i)| (b - i).abs())
        .sum();
    assert!(
        drift < 0.5,
        "zero sociability must not cause directed drift; total drift={drift}"
    );
}
...
fn n8_belief_culture_exposure_weights_dominant_culture() {
    ...
    assert!(
        dist_dom < dist_min,
        "dominant culture must win blend: dist_dom={dist_dom}, dist_min={dist_min}"
    );
}
