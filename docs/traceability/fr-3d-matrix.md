...
| FR-CIV-BEVY-025 | P-W1 kickoff **item 50**: `live_pick` ray–AABB helpers + nearest-entity pick; `just civis-3d-live-smoke` runs `live_pick::` lib tests (headless smoke gate). | `clients/bevy-ref` (`live_pick`, `justfile`, `clients/bevy-ref/README.md`) | `cargo test -p civ-bevy-ref --features bevy --lib live_pick::`, `just civis-3d-live-smoke` | implemented |
...
| FR-CIV-BEVY-027 | P-W1 kickoff **item 52**: live attach polish — `civ-bevy-window` wires `LivePickPlugin`, minimap `MinimapRoot`, HUD `selected_live`; `live_attach` mirrors selection + connection toasts; smoke/README cover `live_focus` + `live_minimap`. | `clients/bevy-ref` (`bevy_window`, `live_attach`, `live_focus`, `live_minimap`, `justfile`, `README.md`) | `just civis-3d-live-smoke`, `cargo test -p civ-bevy-ref --features bevy --lib live_focus::`, `cargo test -p civ-bevy-ref --lib live_minimap::` | implemented |
| FR-CIV-BEVY-028 | P-W1 kickoff **item 53**: `civ-server` tick broadcast emits six `F3D0` frames per tick — voxel, building, agent, civilian state, faction state, event feed (`build_frame_bundle`). | `crates/server` (`ws_bridge`), `crates/protocol-3d` | `cargo test -p civ-server frame_`, `just civis-3d-live-smoke`, `ws_smoke` bundle assertions | implemented |
| FR-CIV-BEVY-029 | P-W1 kickoff **item 54**: apply `Frame3d::EventFeed` in Bevy live attach — `live_stream::apply_event_feed_frame` maps `EventFeedMessage3d` → egui toasts; `live_scene` + `live_attach` on server attach; `bevy_window` logs only. | `clients/bevy-ref` (`live_stream`, `live_scene`, `live_attach`, `bevy_window`) | `cargo test -p civ-bevy-ref --features bevy,egui --lib live_stream::`, `just civis-3d-live-smoke` | implemented |
| FR-CIV-BEVY-030 | P-W1 kickoff **item 55**: apply `Frame3d::CivilianState` / `FactionState` to HUD population + faction chips; `live_stream` merge helpers; `LiveHudSnapshot` overlay `P`/`F` counts; `live_attach` + `civ-bevy-window` wire frames. | `clients/bevy-ref` (`live_stream`, `live_scene`, `live_attach`, `game_ui`, `bevy_window`, `lib.rs`) | `cargo test -p civ-bevy-ref --features bevy --lib live_stream::`, `cargo check -p civ-bevy-ref --features bevy,egui --bin civ-standalone`, `just civis-3d-live-smoke` | implemented |
| FR-CIV-BEVY-031 | P-W1 kickoff **item 56**: `pbr-textures` feature wires `materials.rs` PBR biome loader; `BiomeMaterialsPlugin` on `civ-standalone` sandbox only; height-band terrain material via `pbr_biome_at_height`. | `clients/bevy-ref` (`materials`, `Cargo.toml`, `lib.rs`, `bin/standalone.rs`, `terrain.rs`, `README.md`) | `cargo check -p civ-bevy-ref --features bevy,egui,pbr-textures --bin civ-standalone`, `cargo test -p civ-bevy-ref --features pbr-textures --lib materials::` | implemented |
| FR-CIV-BEVY-032 | P-W1 kickoff **item 57**: extend `just civis-3d-live-smoke` / CI for egui modules (`event_feed`, `menus`) and protocol extended frames (`civilian_state`, `event_feed` round-trips). | `justfile`, `.github/workflows/civis-3d-live-smoke.yml`, `clients/bevy-ref/README.md`, `crates/protocol-3d` | `just civis-3d-live-smoke`, `cargo test -p civ-bevy-ref --features bevy,egui --lib event_feed::`, `cargo test -p civ-bevy-ref --features bevy,egui --lib menus::`, `cargo test -p civ-protocol-3d civilian_state`, `cargo test -p civ-protocol-3d event_feed` | implemented |
| FR-CIV-BEVY-033 | P-W1 kickoff **item 58**: live viewport pick → egui inspector — `LiveStreamScene::civilian_entries` from `CivilianState` frames; `sync_live_selection_to_inspector` fills `SelectedEntityDetails` (name, profession, species, health, needs); reuse `inspector_ui` on server attach. | `clients/bevy-ref` (`live_stream`, `live_attach`, `game_ui`) | `cargo test -p civ-bevy-ref --features bevy,egui --lib game_ui::`, `cargo test -p civ-bevy-ref --features bevy --lib live_stream::`, `cargo check -p civ-bevy-ref --features bevy,egui --bin civ-standalone`, `just civis-3d-live-smoke` | implemented |
| FR-CIV-BEVY-034 | P-W1 kickoff **item 59**: `DiplomacyUiPlugin` on `civ-standalone`; `live_stream::sync_diplomacy_from_faction_frame` maps `FactionStateEntry` → `DipFaction` (population from optional counts or treasury stub); neutral relations matrix until diplomacy sim; toggle panel with **G**. | `clients/bevy-ref` (`diplomacy_ui`, `live_stream`, `live_attach`, `standalone`, `lib.rs`) | `cargo test -p civ-bevy-ref --features bevy,egui --lib diplomacy_ui::`, `just civis-3d-live-smoke` | implemented |
| FR-CIV-BEVY-035 | P-W1 kickoff **item 60**: F3D0 binary encode/decode round-trip for all six `Frame3d` kinds; `parse_ws_payload` decodes each kind (binary + JSON). | `crates/protocol-3d` (`lib.rs` tests), `clients/bevy-ref` (`lib.rs` tests), `justfile` | `cargo test -p civ-protocol-3d frame_bundle_binary_roundtrip_all_kinds`, `cargo test -p civ-bevy-ref --lib parse_ws_payload_decodes_all_frame_kinds`, `just civis-3d-live-smoke` | implemented |
| FR-CIV-BEVY-037 | P-W1 kickoff **item 62**: extend `just civis-3d-live-smoke` / CI for optional Bevy features — `gpu_features` capability defaults (`bevy`) and `materials` biome helpers (`pbr-textures`; headless lib tests, no GPU window). | `justfile`, `.github/workflows/civis-3d-live-smoke.yml`, `clients/bevy-ref/README.md`, `README.md`, `docs/development-guide/p-w1-kickoff.md` | `just civis-3d-live-smoke`, `cargo test -p civ-bevy-ref --features bevy --lib gpu_features::`, `cargo test -p civ-bevy-ref --features pbr-textures --lib materials::` | implemented |
| FR-CIV-BEVY-036 | P-W1 kickoff **item 61**: pause Settings UI shows detected GPU capabilities (`backend_name`, `max_vram_mb`, ray tracing / DLSS / FSR flags) from `GpuCapabilities`; graceful readout when resource missing (headless / pre-startup). | `clients/bevy-ref` (`menus`, `gpu_features`) | `cargo check -p civ-bevy-ref --features bevy,egui --bin civ-standalone`, `cargo test -p civ-bevy-ref --features bevy,egui --lib menus::` | implemented |
| FR-CIV-BEVY-040 | P-W1 kickoff **item 65**: `civ-bevy-window` HUD overlay shows recent `Frame3d::EventFeed` text (`LiveHudSnapshot::last_event`, `live_stream::push_event_feed_to_hud_summary`; no egui). | `clients/bevy-ref` (`lib.rs`, `live_stream`, `bevy_window`) | `cargo check -p civ-bevy-ref --features bevy --bin civ-bevy-window`, `cargo test -p civ-bevy-ref --features bevy --lib` | implemented |
| FR-CIV-BEVY-038 | P-W1 kickoff **item 63**: `AgentAppearanceUpdate.agent_id` uses `civilian.id` (same keys as `CivilianStateEntry.id`) so live pick markers and inspector lookup stay aligned. | `crates/server` (`ws_bridge`) | `cargo test -p civ-server agent_appearance_ids_match_civilian_state_ids`, `cargo test -p civ-server ws_bridge` | implemented |
| FR-CIV-BEVY-039 | P-W1 kickoff **item 64**: client live pick resolves `CivilianStateEntry` via `live_stream::resolve_civilian_for_live_pick` (stable id + legacy fallback when `scene.agents` key differs); `live_attach` + `game_ui` inspector use resolver. | `clients/bevy-ref` (`live_stream`, `live_attach`, `game_ui`) | `cargo test -p civ-bevy-ref --features bevy,egui --lib live_stream::`, `cargo test -p civ-bevy-ref --features bevy,egui --lib game_ui::`, `just civis-3d-live-smoke` | implemented |
| FR-CIV-BEVY-041 | P-W1 kickoff **item 66**: `civ-bevy-window` HUD one-line pick detail — `format_live_pick_hud_line` via `resolve_civilian_for_live_pick`; `LiveHudSnapshot::pick_detail` in `format_overlay`; `sync_live_pick_detail` each frame (no egui). | `clients/bevy-ref` (`live_stream`, `lib.rs`, `bevy_window`) | `cargo test -p civ-bevy-ref --features bevy --lib live_stream::`, `cargo check -p civ-bevy-ref --features bevy --bin civ-bevy-window` | implemented |
| FR-CIV-BEVY-042 | P-W1 kickoff **item 67**: documentation closure for live attach items 58 + 63–67 — `p-w1-kickoff.md`, `fr-3d-matrix.md`, `clients/bevy-ref/README.md` six-frame handler table + agent/civilian id note; smoke/CI comment cross-links. | `docs/development-guide/p-w1-kickoff.md`, `docs/traceability/fr-3d-matrix.md`, `clients/bevy-ref/README.md`, `justfile`, `.github/workflows/civis-3d-live-smoke.yml` | `just civis-3d-live-smoke` (doc-only; no new runtime tests) | implemented |
...
| FR-CIV-PROTO3D-014 | All six `Frame3d` kinds round-trip through the F3D0 binary envelope. | `crates/protocol-3d/` | `frame_bundle_binary_roundtrip_all_kinds` | implemented |
