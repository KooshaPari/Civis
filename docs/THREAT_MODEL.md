...
1. **Local operator ↔ civ-server** — default local-first; `require_role` is param-asserted (not
   AuthN). Spoofable if exposed on a network. Replay HTTP import/export is therefore loopback-only;
   non-loopback binds return `403` until a real authenticated session layer exists.
