...
# Only same-target Rust/Cargo owners are relevant. Unrelated archival or copy
# processes may mention a parent G: path without owning Cargo's target tree.
...
$admitted = $free -ge $required -and $owners.Count -eq 0
...
if (-not $Execute -or -not $admitted) {
    [pscustomobject]@{receipt=$receiptPath;state=$record.state;admitted=$record.admitted;budget=$record.budget;ownerCount=$owners.Count} | ConvertTo-Json -Depth 5
...
[pscustomobject]@{receipt=$receiptPath;state=$record.state;admitted=$record.admitted;childPid=$record.childPid;exitCode=$record.exitCode;stdoutLog=$stdoutPath;stderrLog=$stderrPath} | ConvertTo-Json -Depth 5
