...
impl PowerRegistry {
    ...
    /// Register a single power. Returns `ForbiddenField` if
    /// `coupling_note` mentions a `FORBIDDEN_TARGET_FIELDS` token,
    /// or `DuplicateId` if the id is already in the registry.
    ...
    /// Build a registry from an iterator of `PowerDef`, registering
    /// each in order and returning the first error if any.
