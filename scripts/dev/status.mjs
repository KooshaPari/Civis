...
function render(state, prs, alerts) {
  ...
  lines.push(`clean:     ${state.clean ? "yes" : `no (${state.dirtyFiles} dirty files)`}`);
  if (state.upstream) {
    lines.push(`divergence: ${state.ahead} ahead / ${state.behind} behind ${state.upstream}`);
  }
