...
impl Default for CivHistory {
    fn default() -> Self {
        Self {
            ...
            treasury:      VecDeque::with_capacity(HISTORY_CAP),
...
fn sample_history(hud: Res<crate::HudState>, mut hist: ResMut<CivHistory>) {
    ...
    CivHistory::push(&mut hist.treasury, hud.snapshot.treasury);
...
fn sparkline(buf: &VecDeque<impl Into<f64> + Copy>) -> String {
    ...
    if buf.is_empty() { return "\u{2014}".to_string(); }
...
fn stat_row<T: Into<f64> + Copy + std::fmt::Display>(
    ...
    ui.horizontal(|ui| {
        ...
        if let Some(cur) = current {
            ui.label(egui::RichText::new(format!("  {cur}  [{mn:.1}\u{2013}{mx:.1}]")).monospace()
...
fn draw_history_panel(
    ...
    egui::Window::new("Civilization History")
        .fixed_pos(egui::pos2(screen.center().x - 280.0, 60.0))
        .fixed_size([560.0, 230.0])
        .collapsible(false)
        ...
        .show(ctx, |ui| {
            ...
            stat_row(ui, "Treasury", &hist.treasury);
