...
public static class SafeFileIO
{
    ...
    public static string ReadText(string path) => File.ReadAllText(path, StrictUtf8);

    /// <summary>Read text file lines as UTF-8; throws on invalid bytes.</summary>
    public static string[] ReadAllLines(string path) => File.ReadAllLines(path, StrictUtf8);

    /// <summary>Write text file as UTF-8 (no BOM).</summary>
    public static void WriteText(string path, string content) => File.WriteAllText(path, content, StrictUtf8);
}
