...
impl CaGrid {
    ...
    /// Returns the dirty chunk indices in sorted order.
    ...
    /// Marks the chunk containing `(x, y, z)` as dirty when in bounds.
    ...
    /// Marks every chunk containing a mobile material as dirty.
    ...
    /// Returns the chunk counts in `[x, y, z]` order for 16³ chunks.
        ...
        for y in 0..grid.dims[1] {
            for x in 0..grid.dims[0] {
                ...
                if x == 0
                    ...
                    && matches!(phase_of(reg, id), Phase::Liquid | Phase::Gas)
                {
                    grid.cells[idx] = AIR;
...
pub fn step_n_with_config(
    ...
    for tick in 0..n {
        if !step_with_parity(grid, reg, tick, boundary, sea_level, tick) {
            break;
        }
    }
}
