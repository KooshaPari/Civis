...
mod fr_fr_civ_emergence_005 {
    ...
    fn verify_fr_civ_emergence_005_basic() {
        ...
        let result = fit.fit_power_law(&h);
        // Zipf has alpha ~1.0, R² > 0.8
        assert!(result.alpha > 0.5, "Zipf should have positive alpha, got {}", result.alpha);
        assert!(result.r_squared > 0.5, "Zipf should have decent R², got {}", result.r_squared);
    }
    ...
    fn uniform_distribution_low_alpha() {
        ...
        assert!(result.alpha.abs() < 1.0, "uniform should have low alpha, got {}", result.alpha);
    }
