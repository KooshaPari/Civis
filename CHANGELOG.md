...
### Fixed
- **Dangling `tools/phenotype-journeys` submodule gitlink** — removed the uninitialized submodule gitlink (mode 160000) and its `.gitmodules` entry; the upstream repo is research-only/vaporware and the broken pointer blocked `git submodule`/`status` operations.
