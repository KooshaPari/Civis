...
use civ_agents::{
    ...
    AgentAction, Alignment, Civilian as AgentCivilian, ClusterId, ClusterMember, CohortStats,
...
use crate::religion::{apply_big_gods_response, ReligiousProfile, SubstrateGradients};
    ...
    for (faction, (center_x, center_y)) in faction_capitals.into_iter().enumerate() {
        for _ in 0..CIVILIANS_PER_FACTION {
            ...
            spawn_civilian_at(
                world,
                next_civilian_id,
                Alignment::Faction(faction as u32),
                norm_x,
                norm_y,
...
fn economy_state_from_world(world: &WorldState) -> EconomyState {
    let energy_budget_joules = world.energy_budget_joules.to_bits() / crate::SCALE;
...
impl Simulation {
    ...
    /// Apply a player-issued diplomacy action to the active relation matrix.
    pub fn apply_player_diplomacy_action(
        &mut self,
        source_faction: u32,
        target_faction: u32,
        kind: DiplomacyKind,
    ) -> Option<DiplomacyOutcome> {
        if source_faction == target_faction
            || !self.state.faction_resources.contains_key(&source_faction)
            || !self.state.faction_resources.contains_key(&target_faction)
        {
            return None;
        }
        ...
        let signal = match kind {
            DiplomacyKind::TradeAgreement => DiplomacySignal {
                trade_volume: 3.0,
                need_complementarity: 1.0,
                ..DiplomacySignal::default()
            },
            DiplomacyKind::Conflict => DiplomacySignal {
                combat_grievance: 3.0,
                resource_competition: 1.0,
                ..DiplomacySignal::default()
            },
            DiplomacyKind::Peace => DiplomacySignal {
                trade_volume: 1.5,
                need_complementarity: 1.5,
                ..DiplomacySignal::default()
            },
        };
        let outcome = self.faction_relations.apply_signal(
            ClusterId(source_faction as u64),
            ClusterId(target_faction as u64),
            signal,
        );
        self.push_diplomacy_event(DiplomacyEvent {
            tick: self.state.tick,
    ...
    fn phase_unrest(&mut self) {
        ...
        for &sid in self.settlement_gini.keys() {
            settlement_ids.insert(sid);
        }
        for &sid in self.actor_settlement.values() {
            settlement_ids.insert(sid);
        }
        ...
        for &settlement_id in &settlement_ids {
            ...
            if prev_level < level && level > UnrestLevel::Stable {
    ...
    fn phase_economy(&mut self) {
        ...
        self.economy_state.energy_budget_joules =
            self.state.energy_budget_joules.to_bits() / crate::SCALE;
        ...
        self.state.energy_budget_joules = Fixed::from_bits(
            self.economy_state
                .energy_budget_joules
                .saturating_mul(crate::SCALE),
        );
...
fn settlement_trade_contact_signal(settlement_id: u32, flows: &[SettlementTradeFlow]) -> f32 {
    let volume: i64 = flows
        .iter()
        .filter(|flow| flow.from_settlement == settlement_id || flow.to_settlement == settlement_id)
...
fn scarcity_pressure_signal(energy_budget: Fixed, ra: &Resources, rb: &Resources) -> f32 {
    ...
    let budget_scarce = energy_budget.to_bits() / crate::SCALE < SCARCITY_GATE;
    let a_scarce = ra.energy.to_bits() / crate::SCALE < SCARCITY_GATE
        && ra.food.to_bits() / crate::SCALE < SCARCITY_GATE;
    let b_scarce = rb.energy.to_bits() / crate::SCALE < SCARCITY_GATE
        && rb.food.to_bits() / crate::SCALE < SCARCITY_GATE;
...
mod compat_state_tests {
    ...
    #[test]
    #[ignore = "stale internal assertion after unrest threshold recalibration"]
