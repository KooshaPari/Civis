...
impl BuildingTier {
    ...
    /// Number of output slots a building may produce per tick.
    #[must_use]
...
impl ProductionChain {
    ...
    pub const fn outputs(self) -> &'static [ProductionGood] {
        match self {
            ...
            ProductionChain::Factory => {
                // Energy/Tools pair; represented through Tools here because the
                // current economy `Good` enum has no Energy variant. The
                // surface stays stable; deeper stock math is plugged in once
                // `EconomyState::stocks_mut()` lands.
...
/// spec is captured so that downstream consumers can derive per-tick outputs
...
impl BuildSite {
    ...
    ///
    /// Returns the [`ProductionEvent`]s emitted. Each produced good is one
    /// event; a halt is one event. Empty when the site is incomplete or when
    /// the live `EconomyState` lacks the stock plumbing needed to debit
    /// inputs (the deeper stock accounting is plugged in once
    /// `EconomyState::stocks_mut()` lands in the economy crate).
    pub fn produce_and_collect(
        ...
        // TODO(civ-economy): replace this stub with real per-good stock
        // debit/credit once `EconomyState::stocks_mut()` exposes a
        // `&mut civ_economy::Stocks`. Until then, we emit the produced-good
        // events without touching the economy, and we deliberately do not
        // emit `Halted` events (we have no way to inspect stock levels).
...
mod tests {
    ...
    /// FR-CIV-BUILD-001 — BuildingTier scaling is monotonically non-decreasing.
    #[test]
    fn build_001_tier_scaling_monotonic() {
        assert!(BuildingTier::Primitive.input_slots() < BuildingTier::Advanced.input_slots());
        assert!(BuildingTier::Primitive.base_joules() < BuildingTier::Advanced.base_joules());
        assert!(
    ...
    /// FR-CIV-BUILD-002 — Workshop chain consumes Wood, produces Tools.
    #[test]
    ...
    #[test]
    fn build_003_override_zero_rate_rejected() {
    ...
    /// returns Some(CompletedBuilding) on the finishing tick only.
    #[test]
    fn build_site_progress_and_complete() {
        let id = BuildingId(7);
        let spec = BuildingSpec::minimal(BuildingTier::Primitive, ProductionChain::Farm);
        let mut site = BuildSite::new(id, spec, WorldCoord { x: 0, y: 0, z: 0 });

        let ticks = BuildingTier::Primitive.construction_ticks();
        for i in 0..(ticks - 1) {
            assert!(!site.is_complete());
            assert!(site.tick().is_none(), "tick {i} should not complete");
        }
        let completion = site.tick();
        assert!(completion.is_some());
        assert_eq!(completion.unwrap().id, id);
        assert!(site.is_complete());
        // Subsequent ticks return None.
        assert!(site.tick().is_none());
    }

    /// BuildingGraph::record_completed is idempotent by id.
    #[test]
    fn record_completed_idempotent() {
        let id = BuildingId(11);
        let spec = BuildingSpec::minimal(BuildingTier::Artisan, ProductionChain::Workshop);
        let mut site = BuildSite::new(id, spec, WorldCoord { x: 1, y: 1, z: 1 });
        site.complete();
