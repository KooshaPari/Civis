...
- 2026-06-25 T34: BUILD ROOT CAUSE FOUND — it WAS working (334 deps compiled, bevy huge tree, no errors); my repeated attempts kept getting CUT OFF at ~1-2min by the bg-wrapper before bevy's ~15-20min cold compile finished, and my grep filters (excluding "Compiling") made progress look empty + EXIT=1 was the filtered pipe not cargo. FIX: nohup DETACHED build (survives wrapper cutoff) → /tmp/sa-final.log + Monitor bs85hldiq watching for Finished/error. LESSON: long cold bevy builds need detached+Monitor, not bg-Bash (which cuts off); never filter out "Compiling" when diagnosing build progress. NEXT: Monitor fires Finished → binary at E:/civis-sa-build/debug/civ-standalone.exe → relay launch to user.
